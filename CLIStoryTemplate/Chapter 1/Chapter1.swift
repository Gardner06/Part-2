...
func chapterOne() {
    let storyIntroduction = "Once upon a time there was a little girl named \(String(describing: littleGirl))) that loved to play with her toys \(initialLocation)."
        let accidentLocation = "Bottom of the basement stairs"
        let reason = "Her toy fell down"
        let accidentalDescription = "Then one day, \(reason) the \(toyLocation[0]) stairs all the way to the \(accidentLocation)."
    let consequence = "Because of that, \(String(describing: littleGirl)) went down into the dark \(toyLocation[0]) to get her toy."
